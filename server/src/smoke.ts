...
const CHAT = 999; // fake chat id — a real users row is auto-created (and cleaned) for it
let pass = 0;
let fail = 0;
type Action = { name: string | null; args: Record<string, unknown> };
// first condition of a (bulk-capable) create_alert action
const cond = (a: Action, i = 0) =>
  (((a.args.alerts as unknown[]) ?? [])[i] ?? {}) as Record<string, unknown>;
const alertCount = (a: Action) => ((a.args.alerts as unknown[]) ?? []).length;
...
async function executors() {
  console.log("\n=== EXECUTORS (Postgres + Redis, live prices) ===");
  const user = await findOrCreateByChatId(CHAT);
  const btc = (await getPrice("BTC")).price;

  const c1 = await createAlert(user, { kind: "absolute", symbol: "BTC", op: "below", value: Math.round(btc * 0.9) } as Condition);
  check("create BTC below -10%", c1.ok, c1.ok ? describeAlert(c1.alert) : c1.reason);

  const c2 = await createAlert(user, { kind: "absolute", symbol: "BTC", op: "below", value: Math.round(btc * 1.1) } as Condition);
  check("reject already-true threshold", !c2.ok, c2.ok ? "was created!" : c2.reason);

  const c3 = await createAlert(user, { kind: "pct_change", symbol: "ETH", dir: "down", pct: 5, window: { value: 2, unit: "h" } } as Condition);
  check("create ETH -5%/2h anchored", c3.ok && Math.abs(c3.alert.anchorPrice - c3.current) < 1e-9, c3.ok ? describeAlert(c3.alert) : "");

  check("list shows 2", (await listAlerts(user.userId)).length === 2);
  const delOk = c1.ok && (await deleteAlert(user.userId, c1.alert.id));
  check("delete works", delOk === true && (await listAlerts(user.userId)).length === 1);
  check("delete again -> not found", c1.ok && (await deleteAlert(user.userId, c1.alert.id)) === false);
  check("delete bogus id -> not found", (await deleteAlert(user.userId, "00000000-0000-0000-0000-000000000000")) === false);
  ...
  const na = await createAlert(user, { kind: "absolute", symbol: "BTC", op: "above", value: Math.round(btc * 1.2) } as Condition);
  ...
  const ra = await createAlert(user, { kind: "absolute", symbol: "RELIANCE", op: "above", value: Math.round(rel * 1.2) } as Condition);
  ...
  const g1 = await createAlert(user, { kind: "pct_change", symbol: "BTC", dir: "up", pct: 1, window: { value: 2, unit: "m" } } as Condition);
  check("2-min window rejected (min 5m)", !g1.ok, g1.ok ? "was created!" : g1.reason);
  const niftyOpen = isMarketOpen("NIFTY");
  const g2 = await createAlert(user, { kind: "pct_change", symbol: "NIFTY", dir: "up", pct: 1, window: { value: 2, unit: "h" } } as Condition);
  check(`NIFTY %-alert while market ${niftyOpen ? "open -> ok" : "closed -> rejected"}`, g2.ok === niftyOpen, g2.ok ? "created" : g2.reason);

  // cleanup: cancel everything the smoke user still has (keeps DB/Redis tidy across runs)
  for (const a of await listAlerts(user.userId)) await deleteAlert(user.userId, a.id);
  check("cleanup: smoke user has 0 alerts left", (await listAlerts(user.userId)).length === 0);
}

async function brainTough() {
  console.log("\n=== BRAIN (tough) ===");
  await brainCase("ping me if ripple dips under 90 cents", (a) => a.name === "create_alert" && cond(a).symbol === "XRP" && cond(a).op === "below" && cond(a).value === 0.9, "XRP below 0.9");
  await brainCase("btc dips below 55k", (a) => a.name === "create_alert" && cond(a).value === 55000, "55k -> 55000");
  await brainCase("cardano drops 3% in 45 min", (a) => a.name === "create_alert" && cond(a).symbol === "ADA" && cond(a).dir === "down", "ADA pct down");
  ...
  await brainCase(
    ...
    (a) => a.name === "create_alert" && cond(a).symbol === "TSLA" && cond(a).op === "below" && cond(a).value === 300,
...
async function sessionDerived() {
  ...
  // round 2: from the first live persistence test — direction ambiguity + multi-symbol %
  console.log("\n=== SESSION-DERIVED ROUND 2 (direction & multi-symbol %) ===");
  const win = (a: Action, i = 0) => (cond(a, i).window ?? {}) as { value?: number; unit?: string };
  await brainCase(
    "ping me zomato and swiggy prises by 0.1 % in next 5 min",
    (a, msg) => a.name === null && /up|down|direction|rise|drop/i.test(msg),
    "'by 0.1%' no direction -> ASK (never both dirs)",
  );
  await brainCase(
    "ping me oil and btc prises by 0.1 % in next 5 min also give me there current prices",
    (a) => a.name === null,
    "ambiguous direction wins over mixed intent -> ask first",
  );
  await brainCase(
    "alert me when eth and oil goes above 0.1 % in 5 min",
    (a) => a.name === "create_alert" && alertCount(a) === 2 && cond(a).dir === "up" && cond(a, 1).dir === "up" && cond(a).pct === 0.1,
    "multi-symbol 'goes above N%' -> 2 UP pct alerts",
  );
  await brainCase(
    "alert me if btc and eth rise 1% in 1h and also give me their current prices",
    (a) => a.name === "create_alert" && alertCount(a) === 2,
    "mixed price+alert (direction present) -> create both",
  );
  await brainCase(
    "eth and sol drop 1% in 30 min",
    (a) => a.name === "create_alert" && alertCount(a) === 2 && cond(a).dir === "down" && cond(a, 1).dir === "down",
    "multi-symbol drop -> 2 DOWN pct alerts",
  );
  await brainCase(
    "btc moves 2% in 1h",
    (a, msg) => a.name === null && !/something went wrong/i.test(msg),
    "'moves 2%' no direction -> precise ask (not generic)",
  );
  await brainCase(
    "alert me if btc drops by 5% today",
    (a) => a.name === "create_alert" && cond(a).dir === "down" && win(a).unit === "d",
    "'drops BY 5%' has direction -> create (1d window)",
  );
  await brainCase(
    "btc above 100k and eth above 5k",
    (a) => a.name === "create_alert" && alertCount(a) === 2 && cond(a).value === 100000 && cond(a, 1).value === 5000,
    "2 absolutes, values distributed correctly",
  );
  await brainCase(
    "oil rises 0.5% in 10 min",
    (a) => a.name === "create_alert" && cond(a).symbol === "OIL" && cond(a).dir === "up",
    "OIL % with direction -> create (market gate is service-side)",
  );
  await brainCase(
    "alert me if btc goes up or down 2% in 1h",
    (a) => a.name === null,
    "explicit either-way -> coming soon (not both dirs)",
  );
}
...
async function naiveUsers() {
  ...
  await brainCase("btc 1 lakh cross kare to batana", (a) => a.name === "create_alert" && cond(a).value === 100000 && cond(a).op === "above", "Hinglish + lakh -> above 100000");
  await brainCase("yo alert me when doge moons 10% today", (a) => a.name === "create_alert" && cond(a).dir === "up" && cond(a).pct === 10, "'moons 10% today'");
  await brainCase("notify when tesla crashes 8% in next 4 hours", (a) => a.name === "create_alert" && cond(a).symbol === "TSLA" && cond(a).dir === "down" && cond(a).pct === 8, "'crashes' -> down");
  await brainCase("when eth touches 2k lemme know", (a) => (a.name === "create_alert" && cond(a).value === 2000) || a.name === null, "'touches 2k' (create or ask)");
  await brainCase("set alert btc >65000", (a) => a.name === "create_alert" && cond(a).op === "above" && cond(a).value === 65000, "'>' operator");
  await brainCase("btc < 55k alert pls", (a) => a.name === "create_alert" && cond(a).op === "below" && cond(a).value === 55000, "'<' + k");
  await brainCase("remind me to buy eth at 1500", (a) => a.name === "create_alert" && cond(a).op === "below" && cond(a).value === 1500, "'buy at' -> below");
  await brainCase("0.5% drop on btc in 30 mins", (a) => a.name === "create_alert" && cond(a).pct === 0.5 && cond(a).dir === "down", "decimal percent");
  await brainCase("eth above 4k and btc above 70k", (a) => a.name === "create_alert" && alertCount(a) === 2 && cond(a, 1).value === 70000, "two alerts -> BOTH created (bulk)");
  await brainCase("set alerts on all indian stocks if they rise 5% in 1 hr", (a) => a.name === "create_alert" && alertCount(a) >= 10, "bulk: ALL indian stocks");
  await brainCase("alert me if prices of top 6 crypto rise by 5% in next hour", (a) => a.name === "create_alert" && alertCount(a) === 6 && cond(a).symbol === "BTC", "bulk: top 6 crypto");
  ...
  await brainCase("alert me when tcs goes above 4000", (a) => a.name === "create_alert" && cond(a).symbol === "TCS" && cond(a).value === 4000, "TCS alert (Indian)");
  await brainCase("alert me when reliance crosses 1500", (a) => a.name === "create_alert" && cond(a).symbol === "RELIANCE" && cond(a).value === 1500, "reliance 1500 = ₹ (no USD-ask)");
  ...
  await brainCase(
    "yes do that",
    (a) => a.name === "create_alert" && cond(a).op === "above" && cond(a).value === 65000,
